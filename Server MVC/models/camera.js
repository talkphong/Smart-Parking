...
// callback(isValid?, loaithe, bienso, id_khachhang, isInside?)
function checkSothe(sothe, callback) {
    const sql = `SELECT the.sothe, the.loaithe, xecudan.bienso, khachhang.id_khachhang, xecudan.inside
                FROM the LEFT JOIN xecudan ON xecudan.sothe = the.sothe 
                LEFT JOIN khachhang ON the.id_khachhang = khachhang.id_khachhang
                WHERE the.sothe = '${sothe}' AND the.active = '1';`
    db.query(sql, (err, results) => {
        ...
        if (results.length == 0) {callback(err, false, null, null, null, null)} 
...
function delete_xevanglai(bienso){
    db.query('UPDATE xevanglai SET xevanglai.active = ?', 0, function(err, data){
        if (err) {console.error(err)}
